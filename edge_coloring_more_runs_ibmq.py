...
def init_graph(no_of_nodes,edge_probability):
    ...
    while edges != NO_OF_EDGES:
        graph = nx.erdos_renyi_graph(no_of_nodes,edge_probability,seed=None,directed=False)
        edges = graph.number_of_edges()
#    adjacency_matrix=nx.to_numpy_matrix(graph).astype(int)
    incidence_matrix =  nx.incidence_matrix(graph).todense().astype(int)
...
def check_edges_validity(graph, colors):
    ...
    for i in range(NO_OF_NODES):
        for j in range(NO_OF_EDGES):
            if graph[i][j]:
                scolor = colors[j]
                for k in range(j+1,NO_OF_EDGES):
                    if graph[i][k]:
                        if scolor == colors[k]:
                            continue
                        else:
                            no_of_valid_nodes +=1
    return no_of_valid_nodes
...
def get_oracle_instruction(positive_value_array):
    ...
    For subtraction, carry[0] will be 1=> twos complement subtract
    """
    ...
    oracle_circ.x(carry_reg[0]) #reset carry in
    """
    ...
    """
#    oracle_circ.initialize(desired_vector,[carry_reg[q] for q in range(0, NO_OF_QUBITS_FITNESS+1)])
